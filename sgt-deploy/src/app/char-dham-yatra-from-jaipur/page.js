...
function Schema() {
  const faq = {
    ...
    mainEntity: [
      { '@type': 'Question', name: 'How to reach Haridwar from Jaipur for Char Dham Yatra?',
        acceptedAnswer: { '@type': 'Answer', text: 'No direct Jaipur–Dehradun flight. Fly via Delhi or take overnight direct train to Haridwar.' } },
      { '@type': 'Question', name: 'What is the cost of Char Dham Yatra from Jaipur?',
        acceptedAnswer: { '@type': 'Answer', text: 'All-inclusive Char Dham Yatra from Jaipur starts at ₹22,999 per person for 9N/10D from Haridwar.' } },
    ],
  };
    ...
    <>
      ...
      <section style={{ background:'linear-gradient(145deg,var(--navy) 0%,#1A3E75 60%,var(--teal) 100%)', padding:'52px 20px 40px' }}>
        <div style={{ maxWidth:860, margin:'0 auto', textAlign:'center' }}>
          <span style={{ background:'rgba(232,146,10,0.18)', color:'#FFD166', fontSize:11, fontWeight:700, letterSpacing:'0.12em', textTransform:'uppercase', padding:'5px 16px', borderRadius:100, display:'inline-block', marginBottom:14 }}>
            Jaipur · Char Dham 2026
          </span>
          <h1 style={{ color:'#fff', fontFamily:'var(--font-display)', fontSize:'clamp(1.7rem,4vw,2.6rem)', fontWeight:700, letterSpacing:'-0.02em', marginBottom:14 }}>
            Char Dham Yatra Package from Jaipur 2026
          </h1>
          <p style={{ color:'rgba(255,255,255,0.8)', fontSize:15, lineHeight:1.7, maxWidth:680, margin:'0 auto 20px' }}>
            Yamunotri · Gangotri · Kedarnath · Badrinath &mdash; from <strong style={{ color:'#FFD166' }}>{`₹22,999/person`}</strong> · Direct operator · Zero commission
          </p>
        </div>
      </section>
      ...
      <article style={{ maxWidth:900, margin:'0 auto', padding:'36px 20px 60px' }}>
        ...
        <div style={{ background:'var(--navy)', borderRadius:14, padding:'18px 20px', marginBottom:28, display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(140px,1fr))', gap:10 }}>
          {[['Distance to Haridwar','490km'],['Best Route','Via NH48 via Delhi'],['Yatra Duration','9N/10D'],['Total Trip','11-13 days'],['Package From','₹22,999/person'],['Season','Apr 19 – Nov 2026']].map(([k,v])=>(
        ...
        <h2 style={{ fontFamily:'var(--font-display)', fontSize:'1.4rem', fontWeight:700, color:'var(--navy)', marginBottom:14 }}>
          How to Reach Haridwar from Jaipur
        </h2>
        <p style={{ fontSize:16, color:'#334155', lineHeight:1.85, marginBottom:20 }}>
          Jaipur to Haridwar is 490km via NH48 via Delhi. No direct Jaipur–Dehradun flight. Fly via Delhi or take overnight direct train to Haridwar. Haridwar is the gateway to Char Dham Yatra — your package vehicle departs at 4–5 AM from Haridwar.
        </p>

        <h2 style={{ fontFamily:'var(--font-display)', fontSize:'1.4rem', fontWeight:700, color:'var(--navy)', marginBottom:14 }}>
          Trains from Jaipur to Haridwar
        </h2>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13.5 }}>
            ...
            <tbody>
              <tr style={{ borderBottom:'1px solid var(--border)', background:'#fff' }}>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>Dehradun Exp 19020</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>JP→DDN</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>15:45</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>09:45+1</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>SL ₹380</td>
              </tr>
              <tr style={{ borderBottom:'1px solid var(--border)', background:'var(--bg)' }}>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>JP→NDLS+HW Shatabdi</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>Via Delhi</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>Morning</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>9 hrs total</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>₹700 total</td>
              </tr>
              <tr style={{ borderBottom:'1px solid var(--border)', background:'#fff' }}>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>Direct overnight JP→HW</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>JP→HW</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>Night</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>13 hrs</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>SL ₹320</td>
              </tr>
            </tbody>
          </table>
        </div>
