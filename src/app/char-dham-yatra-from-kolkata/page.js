...
export const metadata = {
  title: 'Char Dham Yatra Package from Kolkata 2026 — Distance, Route & Booking',
  description: 'Char Dham Yatra from Kolkata 2026. Distance to Haridwar: 1,520km. Route via NH19 via Varanasi. All-inclusive packages from ₹24,999/person. Direct operator, zero commission.',
...
function Schema() {
  const faq = {
    ...
    mainEntity: [
      { '@type': 'Question', name: 'How to reach Haridwar from Kolkata for Char Dham Yatra?',
        acceptedAnswer: { '@type': 'Answer', text: 'Fly Kolkata to Dehradun (2 hrs, IndiGo/Air India) then taxi to Haridwar.' } },
      { '@type': 'Question', name: 'What is the cost of Char Dham Yatra from Kolkata?',
        acceptedAnswer: { '@type': 'Answer', text: 'All-inclusive Char Dham Yatra from Kolkata starts at ₹24,999 per person for 9N/10D from Haridwar.' } },
    ],
  };
    ...
    <>
      ...
      <section style={{ background:'linear-gradient(145deg,var(--navy) 0%,#1A3E75 60%,var(--teal) 100%)', padding:'52px 20px 40px' }}>
        <div style={{ maxWidth:860, margin:'0 auto', textAlign:'center' }}>
          <span style={{ background:'rgba(232,146,10,0.18)', color:'#FFD166', fontSize:11, fontWeight:700, letterSpacing:'0.12em', textTransform:'uppercase', padding:'5px 16px', borderRadius:100, display:'inline-block', marginBottom:14 }}>
            Kolkata · Char Dham 2026
          </span>
          <h1 style={{ color:'#fff', fontFamily:'var(--font-display)', fontSize:'clamp(1.7rem,4vw,2.6rem)', fontWeight:700, letterSpacing:'-0.02em', marginBottom:14 }}>
            Char Dham Yatra Package from Kolkata 2026
          </h1>
      ...
      <article style={{ maxWidth:900, margin:'0 auto', padding:'36px 20px 60px' }}>
        ...
        <div style={{ background:'var(--navy)', borderRadius:14, padding:'18px 20px', marginBottom:28, display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(140px,1fr))', gap:10 }}>
          {[['Distance to Haridwar','1,520km'],['Best Route','Via NH19 via Varanasi'],['Yatra Duration','9N/10D'],['Total Trip','11-13 days'],['Package From','₹24,999/person'],['Season','Apr 19 – Nov 2026']].map(([k,v])=>(
        ...
        <p style={{ fontSize:16, color:'#334155', lineHeight:1.85, marginBottom:20 }}>
          Kolkata to Haridwar is 1,520km via NH19 via Varanasi. Fly Kolkata to Dehradun (2 hrs, IndiGo/Air India) then taxi to Haridwar. Haridwar is the gateway to Char Dham Yatra — your package vehicle departs at 4–5 AM from Haridwar.
        </p>

        <h2 style={{ fontFamily:'var(--font-display)', fontSize:'1.4rem', fontWeight:700, color:'var(--navy)', marginBottom:14 }}>
          Trains from Kolkata to Haridwar
        </h2>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13.5 }}>
            ...
            <tbody>
              <tr style={{ borderBottom:'1px solid var(--border)', background:'#fff' }}>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>Doon Express 13009</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>HWH→HW</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>20:05</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>23:50+1</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>SL ₹520</td>
              </tr>
              <tr style={{ borderBottom:'1px solid var(--border)', background:'var(--bg)' }}>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>Uttarakhand Exp 19307</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>HWH→HW</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>23:10</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>07:30+2</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>SL ₹530</td>
              </tr>
              <tr style={{ borderBottom:'1px solid var(--border)', background:'#fff' }}>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>HWH-DDN Express</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>HWH→DDN</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>Various</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>30+ hrs</td>
                <td style={{ padding:'9px 12px', color:'#334155', fontSize:13 }}>SL ₹480</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div style={{ background:'var(--navy)', borderRadius:16, padding:'28px 24px', textAlign:'center', marginTop:28 }}>
          <h3 style={{ color:'#fff', fontFamily:'var(--font-display)', fontSize:'1.3rem', marginBottom:10 }}>Book Char Dham Yatra from Kolkata — Free Quote in 2 Hours</h3>
          <p style={{ color:'rgba(255,255,255,0.7)', fontSize:14, marginBottom:18 }}>Direct operator. Zero commission. {SITE.phone}</p>
          <div style={{ display:'flex', gap:10, justifyContent:'center', flexWrap:'wrap' }}>
            <a href={`https://wa.me/${SITE.whatsapp}?text=${encodeURIComponent('Namaste! I am from Kolkata and want to book Char Dham Yatra 2026.')}`} target="_blank" rel="noopener noreferrer"
