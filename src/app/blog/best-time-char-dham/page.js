    ...
    <>
      ...
      <article style={{ maxWidth:860, margin:'0 auto', padding:'40px 20px 60px' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10, marginBottom:28 }}>
          {months.map(m => (
            <div key={m.month} style={{ background:m.color, border:`1px solid ${m.border}`, borderRadius:10, padding:'14px 18px', display:'grid', gridTemplateColumns:'120px 1fr', gap:14, alignItems:'start' }}>
              ...
              <div style={{ fontSize:13.5, color:'var(--text-mid)', lineHeight:1.6 }}>{m.note}</div>
            </div>
          ))}
        </div>
        ...
        <p style={{ ...p, fontSize:13.5, background:'var(--navy-light)', padding:'12px 16px', borderRadius:10 }}>
          📌 <strong>We send WhatsApp alerts</strong> to all our past customers when exact 2026 opening dates are announced. Save our number: <a href={`tel:${SITE.phone}`} style={{ color:'var(--navy)', fontWeight:600 }}>{SITE.phone}</a>
        </p>
