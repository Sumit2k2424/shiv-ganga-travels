    ...
    <>
      ...
      <nav style={{ background:'var(--bg)', borderBottom:'1px solid hsl(var(--border))', padding:'9px 20px' }}>
        <div style={{ maxWidth:'var(--container)', margin:'0 auto', fontSize:12, color:'var(--text-muted)', display:'flex', gap:6, flexWrap:'wrap' }}>
          ...
          <Link prefetch={false} href="/kedarnath-yatra" style={{ color:'var(--teal)', textDecoration:'none' }}>Kedarnath Yatra</Link><span>›</span>
      ...
      <article className="blog-container" itemScope itemType="https://schema.org/Article">
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10, marginBottom:20 }}>
          {[
            ...
            { t:'How to book via IRCTC', d:'Go to heliyatra.irctc.co.in, the only official portal → log in → choose helipad, date and slot → enter each passenger as on their Char Dham yatra registration → pay online. Slots fill within hours during peak May–June season, so book as early as possible. Cancellations are done online on the same portal.' },
            { t:'What is the weight limit?', d:'80 kg per passenger. Every kilo above that costs ₹150, paid at the helipad, and each passenger is weighed separately, so a lighter companion does not offset it. One bag of up to 2 kg is allowed. Leave main luggage in your vehicle at the helipad parking. These rules are printed on every 2026 IRCTC ticket.' },
            ...
            { t:'What if my slot is cancelled?', d:'Flights are cancelled for weather, technical reasons or official orders. The fare is refunded to the account you paid from within 5–7 days, minus payment-gateway charges and the IRCTC convenience fee. You are not moved to the next slot: get the ticket stamped as cancelled at the helipad counter and rebook. This is why we recommend building a buffer day into your Kedarnath itinerary.' },
          ].map(item=>(
            <details key={item.t} style={{ background:'#fff', borderRadius:10, border:'1px solid hsl(var(--border))', overflow:'hidden' }}>
              <summary style={{ padding:'13px 16px', cursor:'pointer', listStyle:'none', fontWeight:600, fontSize:14.5, color:'var(--navy)', display:'flex', justifyContent:'space-between' }}>
                {item.t} <span>＋</span>
