...
export const metadata = {
  title: { absolute: `Char Dham Yatra from Nagpur ${SITE.season} | Direct Flight Available` },
  ...
  twitter: {
    ...
    title: `Char Dham Yatra from Nagpur ${SITE.season}`,
    description: '1,450 km — fly via Dehradun (2 hrs). Char Dham packages from ₹17,200/person. Zero commission, direct Haridwar operator.',
...
function Schema() {
  const faq = {
    ...
    mainEntity:[
      ...
      { '@type':'Question', name:'What is the Char Dham Yatra package cost from Nagpur?', acceptedAnswer:{ '@type':'Answer', text:'Char Dham Yatra from Nagpur starts at ₹17,200/person for a 9N/10D all-inclusive package — AC vehicle, hotels, meals, VIP darshan at all 4 dhams, guide, registration. Zero commission.' } },
    ...
    <>
      ...
      <section style={{ background:'linear-gradient(145deg,var(--navy) 0%,#1A3E75 60%,var(--teal) 100%)', padding:'52px 20px 40px', textAlign:'center' }}>
        <div style={{ maxWidth:860, margin:'0 auto' }}>
          ...
          <p style={{ color:'rgba(255,255,255,0.85)', fontSize:15, lineHeight:1.7, maxWidth:680, margin:'0 auto 20px' }}>
            1,450 km — fly via Dehradun (2 hrs) · All-inclusive from <strong style={{ color:'#FFD166' }}>{ '₹17,200' }/person</strong> · Zero commission
          </p>
      ...
      <article style={{ maxWidth:900, margin:'0 auto', padding:'40px 20px 60px' }}>
        ...
        <div style={{ background:'var(--navy)', borderRadius:14, padding:'18px 20px', marginBottom:28, display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(140px,1fr))', gap:10 }}>
          {[['Distance','1450 km'],['Best route','Fly to Dehradun/Delhi'],['Total trip','11–13 days'],['Package from','₹17,200/person'],['Season 2026','Apr 19 – Nov'],['Registration','FREE · We handle']].map(([k,v])=>(
        ...
        <div style={{ background:'rgba(29,158,117,0.08)', border:'1px solid #1D9E75', borderRadius:10, padding:'12px 16px', marginBottom:24, fontSize:13.5, color:'#0F5132' }}>
          <strong>Package from Nagpur:</strong> ₹17,200/person · 9N/10D · All-inclusive · 5% GST included · Zero commission — you pay us directly
        </div>
        ...
        <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:28 }}>
          <span key='Nagpur Railway Station' style={{ background:'#fff',border:'1px solid hsl(var(--border))',color:'var(--navy)',padding:'7px 14px',borderRadius:8,fontSize:12.5,fontWeight:600 }}>📍 Nagpur Railway Station</span>
          <span key='Dr. Babasaheb Ambedkar International Airport' style={{ background:'#fff',border:'1px solid hsl(var(--border))',color:'var(--navy)',padding:'7px 14px',borderRadius:8,fontSize:12.5,fontWeight:600 }}>📍 Dr. Babasaheb Ambedkar International Airport</span>
        </div>
