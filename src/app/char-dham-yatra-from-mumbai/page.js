...
export const metadata = {
  title: 'Char Dham Yatra Package from Mumbai 2026 — Flight, Train & All-Inclusive Tours',
  description: 'Book Char Dham Yatra from Mumbai 2026. Flight to Dehradun or train to Haridwar. All-inclusive packages from ₹24,999. Delhi/Haridwar pickup. VIP darshan. Trusted since 2010.',
  keywords: ['char dham yatra from mumbai, char dham yatra package from mumbai, mumbai to char dham yatra 2026, char dham tour from mumbai'],
  alternates: { canonical: `${SITE.baseUrl}/char-dham-yatra-from-mumbai` },
  openGraph: { title: 'Char Dham Yatra Package from Mumbai 2026 — Flight, Train & All-Inclusive Tours', description: 'Book Char Dham Yatra from Mumbai 2026. Flight to Dehradun or train to Haridwar. All-inclusive packages from ₹24,999. Delhi/Haridwar pickup. VIP darshan. Trusted since 2010.', url: `${SITE.baseUrl}/char-dham-yatra-from-mumbai`, type:'website' },
};
    ...
    <>
      ...
      <section style={{ background:'linear-gradient(145deg,var(--navy) 0%,var(--navy-mid) 60%,var(--teal) 100%)', padding:'56px 20px 44px', textAlign:'center' }}>
        <div style={{ maxWidth:820, margin:'0 auto' }}>
          <span style={{ background:'rgba(232,146,10,0.18)', color:'#FFD166', fontSize:11, fontWeight:700, letterSpacing:'0.12em', textTransform:'uppercase', padding:'5px 16px', borderRadius:100, display:'inline-block', marginBottom:16 }}>From Mumbai · 2026</span>
          <h1 className="display-title" style={{ color:'#fff', fontSize:'clamp(1.8rem,4.5vw,3rem)', marginBottom:14 }}>Char Dham Yatra Package from Mumbai 2026</h1>
          <p style={{ color:'rgba(255,255,255,0.8)', fontSize:15, lineHeight:1.7 }}>All-inclusive from <strong style={{color:'#FFD166'}}>From ₹24,999</strong> · Pickup from Mumbai · VIP darshan · Zero commission</p>
          <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap', marginTop:24 }}>
            <a href={`https://wa.me/${SITE.whatsapp}?text=${encodeURIComponent('Namaste! I want to book Char Dham Yatra 2026 from Mumbai.')}`} target="_blank" rel="noopener noreferrer"
      ...
      <article style={{ maxWidth:900, margin:'0 auto', padding:'40px 20px 60px' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10, marginBottom:24 }}>
            ...
            <div><div style={{fontWeight:700, fontSize:13.5, color:'var(--navy)', marginBottom:3}}>✈️ Flight to Dehradun</div><div style={{fontSize:13, color:'var(--text-mid)', lineHeight:1.5}}>1h 45min direct · IndiGo/Air India · ₹3,500–7,000 · Jolly Grant Airport, then 1.5 hrs to Haridwar</div></div>
          </div>
          <div style={{display:'flex', gap:14, padding:'12px 16px', background:'#fff', borderRadius:10, border:'1px solid var(--border)', alignItems:'flex-start'}}>
            <span style={{fontSize:20, flexShrink:0}}>🚂</span>
            <div><div style={{fontWeight:700, fontSize:13.5, color:'var(--navy)', marginBottom:3}}>🚂 Train to Haridwar</div><div style={{fontSize:13, color:'var(--text-mid)', lineHeight:1.5}}>Mumbai CST → Haridwar (Dehradun Express) · 24–28 hrs · ₹800–2,500 AC</div></div>
          </div>
          <div style={{display:'flex', gap:14, padding:'12px 16px', background:'#fff', borderRadius:10, border:'1px solid var(--border)', alignItems:'flex-start'}}>
            <span style={{fontSize:20, flexShrink:0}}>🚂</span>
            <div><div style={{fontWeight:700, fontSize:13.5, color:'var(--navy)', marginBottom:3}}>🚂 Train to Rishikesh</div><div style={{fontSize:13, color:'var(--text-mid)', lineHeight:1.5}}>Mumbai → Haridwar then cab to Rishikesh · Best option for budget travel</div></div>
          </div>
        </div>
        <p style={{...p}}>Many Mumbai pilgrims prefer the flight to Dehradun for a 1h 45min journey vs a 24-hour train. We arrange seamless pickups from Jolly Grant Airport and Haridwar railway station.</p>

        <h2 style={{...h2}}>Our Char Dham Package from Mumbai — ₹24,999 Per Person</h2>
        <div style={{ background:'var(--navy-light)', borderRadius:14, padding:'20px 22px', marginBottom:24 }}>
          <div style={{ fontWeight:700, fontSize:14, color:'var(--navy)', marginBottom:14 }}>📦 Char Dham Package from Mumbai includes:</div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))', gap:10 }}>
            {[
              {icon:'🚌', item:'AC private vehicle from Mumbai airport or Haridwar station'},
        ...
        <div style={{ background:'var(--navy)', borderRadius:16, padding:'28px 24px', textAlign:'center' }}>
          <h3 style={{ color:'#fff', fontFamily:'var(--font-display)', fontSize:'1.4rem', marginBottom:10 }}>Book Your Mumbai Char Dham Package</h3>
          <p style={{ color:'rgba(255,255,255,0.75)', fontSize:14, marginBottom:20 }}>Free itinerary within 2 hours. Pickup from Mumbai airport or Haridwar/Dehradun railway station. Zero commission. ₹24,999 all-inclusive.</p>
