...
function Schema() {
  const ld = {
    ...
    name:'Haridwar to Gangotri Cab — Shiv Ganga Travels',
    url:`${SITE.baseUrl}/haridwar-to-gangotri-cab`,
  ...
  const bc = { '@context':'https://schema.org','@type':'BreadcrumbList', itemListElement:[
    ...
    {'@type':'ListItem',position:3,name:'Haridwar to Gangotri',item:`${SITE.baseUrl}/haridwar-to-gangotri-cab`},
  ]};
  ...
  return (<>
    ...
    <section style={{ background:'linear-gradient(145deg,var(--navy) 0%,var(--navy-mid) 60%,var(--teal) 100%)', padding:'52px 20px 40px', textAlign:'center' }}>
      <div style={{ maxWidth:820, margin:'0 auto' }}>
        ...
        <h1 className="display-title" style={{ color:'#fff', fontSize:'clamp(1.6rem,4vw,2.6rem)', marginBottom:12 }}>Haridwar to Gangotri Cab 2026</h1>
        <div style={{ display:'flex', justifyContent:'center', gap:12, flexWrap:'wrap', marginBottom:24 }}>
          {[{icon:'📏',val:'265 km'},{icon:'⏱',val:'7–8 hrs'},{icon:'📍',val:'Via Chamba → Harsil'}].map(c=>(
        ...
        <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap' }}>
          <a href={`https://wa.me/${SITE.whatsapp}?text=${encodeURIComponent('Namaste! I want to book a cab from Haridwar to Gangotri for Char Dham Yatra 2026.')}`} target="_blank" rel="noopener noreferrer"
          ...
          <a href={`tel:${SITE.phone}`} style={{ background:'rgba(255,255,255,0.15)', color:'#fff', padding:'12px 26px', borderRadius:10, fontWeight:700, fontSize:14, textDecoration:'none', border:'1px solid rgba(255,255,255,0.25)' }}>📞 {SITE.phone}</a>
        </div>
      </div>
    </section>
    ...
    <nav style={{ background:'var(--bg)', borderBottom:'1px solid var(--border)', padding:'10px 20px' }}>
      <div style={{ maxWidth:'var(--container)', margin:'0 auto', fontSize:12, color:'var(--text-muted)', display:'flex', gap:6, flexWrap:'wrap' }}>
        ...
        <span>Haridwar to Gangotri</span>
      </div>
    </nav>
    ...
    <div style={{ maxWidth:900, margin:'0 auto', padding:'36px 20px 60px' }}>
      ...
      <div style={{ background:'var(--navy-light)', borderRadius:12, padding:'16px 20px', marginBottom:24, display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))', gap:12 }}>
        {[{icon:'📏',label:'Distance',val:'265 km'},{icon:'⏱',label:'Drive time',val:'7–8 hrs'},{icon:'🏔️',label:'Route',val:'Via Chamba → Harsil'},{icon:'ℹ️',label:'Note',val:'Motor road goes to Gangotri temple (3,41…'}].map(s=>(
      ...
      <p style={{...p}}>
        ...
        <span style={{ fontSize:13.5, color:'var(--text-muted)' }}>Motor road goes to Gangotri temple (3,415m). No trek — unique among the four dhams.</span>
      </p>
      ...
      <div style={{ background:'#fff', borderRadius:12, border:'1px solid var(--border)', overflow:'hidden', marginBottom:28 }}>
        <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13.5 }}>
          ...
          <tbody>
            <tr style={{ borderTop:'1px solid var(--border)' }}><td style={{ padding:'11px 16px', fontWeight:600, color:'var(--navy)' }}>Ertiga</td><td style={{ padding:'11px 16px', textAlign:'center', color:'var(--text-muted)' }}>4 seats</td><td style={{ padding:'11px 16px', textAlign:'right', fontWeight:700, color:'var(--navy)', fontSize:15 }}>₹6,500–7,500</td></tr>
<tr style={{ borderTop:'1px solid var(--border)' }}><td style={{ padding:'11px 16px', fontWeight:600, color:'var(--navy)' }}>Innova Crysta</td><td style={{ padding:'11px 16px', textAlign:'center', color:'var(--text-muted)' }}>6 seats</td><td style={{ padding:'11px 16px', textAlign:'right', fontWeight:700, color:'var(--navy)', fontSize:15 }}>₹7,500–9,000</td></tr>
<tr style={{ borderTop:'1px solid var(--border)' }}><td style={{ padding:'11px 16px', fontWeight:600, color:'var(--navy)' }}>Tempo Traveller 9S</td><td style={{ padding:'11px 16px', textAlign:'center', color:'var(--text-muted)' }}>9 seats</td><td style={{ padding:'11px 16px', textAlign:'right', fontWeight:700, color:'var(--navy)', fontSize:15 }}>₹10,500–12,000</td></tr>
          </tbody>
        </table>
      </div>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:8, marginBottom:28 }}>
        <div style={{ display:'flex', gap:10, padding:'10px 14px', background:'#fff', borderRadius:9, border:'1px solid var(--border)', fontSize:13.5, color:'var(--text-mid)', lineHeight:1.6 }}><span style={{ flexShrink:0 }}>💡</span>Stop at Harsil valley (25km before Gangotri) — stunning Bhagirathi gorge</div>
<div style={{ display:'flex', gap:10, padding:'10px 14px', background:'#fff', borderRadius:9, border:'1px solid var(--border)', fontSize:13.5, color:'var(--text-mid)', lineHeight:1.6 }}><span style={{ flexShrink:0 }}>💡</span>Bhagirathi Shila and Surya Kund are near the temple — plan 3–4 hours at Gangotri</div>
<div style={{ display:'flex', gap:10, padding:'10px 14px', background:'#fff', borderRadius:9, border:'1px solid var(--border)', fontSize:13.5, color:'var(--text-mid)', lineHeight:1.6 }}><span style={{ flexShrink:0 }}>💡</span>Gaumukh glacier trek (20km, permit required) is an add-on from Gangotri</div>
<div style={{ display:'flex', gap:10, padding:'10px 14px', background:'#fff', borderRadius:9, border:'1px solid var(--border)', fontSize:13.5, color:'var(--text-mid)', lineHeight:1.6 }}><span style={{ flexShrink:0 }}>💡</span>Road is smooth via Tehri new bypass — much faster than the old NH</div>
      </div>
      ...
      <div style={{ background:'var(--navy-light)', borderRadius:12, padding:'16px 20px', marginBottom:24 }}>
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
          ...
          ].filter(l=>!l.href.includes('haridwar-to-gangotri-cab')).map(l=>(
      ...
      <div style={{ background:'var(--navy)', borderRadius:16, padding:'24px', textAlign:'center' }}>
        <h3 style={{ color:'#fff', fontFamily:'var(--font-display)', fontSize:'1.3rem', marginBottom:10 }}>Book Haridwar to Gangotri Cab</h3>
