    ...
    <>
      ...
      <section style={{ background:'linear-gradient(145deg,var(--navy) 0%,var(--navy-mid) 60%,var(--teal) 100%)', padding:'56px 20px 44px', textAlign:'center' }}>
        <div style={{ maxWidth:820, margin:'0 auto' }}>
          <span style={{ background:'rgba(232,146,10,0.18)', color:'#FFD166', fontSize:11, fontWeight:700, letterSpacing:'0.12em', textTransform:'uppercase', padding:'5px 16px', borderRadius:100, display:'inline-block', marginBottom:16 }}>From Hyderabad · 2026</span>
      ...
      <article style={{ maxWidth:900, margin:'0 auto', padding:'40px 20px 60px' }}>
        ...
        <p style={{...p}}>Most Hyderabad pilgrims travel to Haridwar first, which is the official starting point of the Char Dham Yatra. From Haridwar, our vehicle picks you up and the journey begins. Here are your best travel options:</p>
        <div style={{ display:'flex', flexDirection:'column', gap:10, marginBottom:24 }}>
            ...
            <div><div style={{fontWeight:700, fontSize:13.5, color:'var(--navy)', marginBottom:3}}>✈️ Hyderabad → Dehradun</div><div style={{fontSize:13, color:'var(--text-mid)', lineHeight:1.5}}>Via Delhi · Total 3–4 hrs · ₹4,500–9,000 · Most convenient</div></div>
          </div>
          <div style={{display:'flex', gap:14, padding:'12px 16px', background:'#fff', borderRadius:10, border:'1px solid var(--border)', alignItems:'flex-start'}}>
            <span style={{fontSize:20, flexShrink:0}}>✈️</span>
            <div><div style={{fontWeight:700, fontSize:13.5, color:'var(--navy)', marginBottom:3}}>✈️ Hyderabad → Delhi</div><div style={{fontSize:13, color:'var(--text-mid)', lineHeight:1.5}}>1h 45min · Then Delhi → Haridwar by train/cab (5 hrs)</div></div>
          </div>
          <div style={{display:'flex', gap:14, padding:'12px 16px', background:'#fff', borderRadius:10, border:'1px solid var(--border)', alignItems:'flex-start'}}>
            <span style={{fontSize:20, flexShrink:0}}>🚂</span>
            <div><div style={{fontWeight:700, fontSize:13.5, color:'var(--navy)', marginBottom:3}}>🚂 Train Hyderabad → Haridwar</div><div style={{fontSize:13, color:'var(--text-mid)', lineHeight:1.5}}>30–36 hrs · Falaknuma Express or connecting trains · Budget option</div></div>
          </div>
        </div>
        <p style={{...p}}>Hyderabad has a large Telugu-speaking pilgrimage community who undertake Char Dham Yatra every year. We have guided hundreds of Hyderabad families and offer Telugu-speaking guide assistance on request.</p>

        <h2 style={{...h2}}>Our Char Dham Package from Hyderabad — ₹25,999 Per Person</h2>
        <div style={{ background:'var(--navy-light)', borderRadius:14, padding:'20px 22px', marginBottom:24 }}>
          <div style={{ fontWeight:700, fontSize:14, color:'var(--navy)', marginBottom:14 }}>📦 Char Dham Package from Hyderabad includes:</div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))', gap:10 }}>
            {[
              {icon:'🚌', item:'AC private vehicle from Hyderabad airport or Haridwar station'},
