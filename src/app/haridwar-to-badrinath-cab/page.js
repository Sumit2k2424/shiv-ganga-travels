...
function Schema() {
  const ld = {
    ...
    name:'Haridwar to Badrinath Cab — Shiv Ganga Travels',
    url:`${SITE.baseUrl}/haridwar-to-badrinath-cab`,
  ...
  const bc = { '@context':'https://schema.org','@type':'BreadcrumbList', itemListElement:[
    ...
    {'@type':'ListItem',position:3,name:'Haridwar to Badrinath',item:`${SITE.baseUrl}/haridwar-to-badrinath-cab`},
  ]};
  ...
  return (<>
    ...
    <section style={{ background:'linear-gradient(145deg,var(--navy) 0%,var(--navy-mid) 60%,var(--teal) 100%)', padding:'52px 20px 40px', textAlign:'center' }}>
      <div style={{ maxWidth:820, margin:'0 auto' }}>
        ...
        <h1 className="display-title" style={{ color:'#fff', fontSize:'clamp(1.6rem,4vw,2.6rem)', marginBottom:12 }}>Haridwar to Badrinath Cab 2026</h1>
        <div style={{ display:'flex', justifyContent:'center', gap:12, flexWrap:'wrap', marginBottom:24 }}>
          {[{icon:'📏',val:'320 km'},{icon:'⏱',val:'8–9 hrs'},{icon:'📍',val:'Via Devprayag → Joshimath'}].map(c=>(
        ...
        <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap' }}>
          <a href={`https://wa.me/${SITE.whatsapp}?text=${encodeURIComponent('Namaste! I want to book a cab from Haridwar to Badrinath for Char Dham Yatra 2026.')}`} target="_blank" rel="noopener noreferrer"
    ...
    <nav style={{ background:'var(--bg)', borderBottom:'1px solid var(--border)', padding:'10px 20px' }}>
      <div style={{ maxWidth:'var(--container)', margin:'0 auto', fontSize:12, color:'var(--text-muted)', display:'flex', gap:6, flexWrap:'wrap' }}>
        ...
        <span>Haridwar to Badrinath</span>
      </div>
    </nav>
    ...
    <div style={{ maxWidth:900, margin:'0 auto', padding:'36px 20px 60px' }}>
      ...
      <div style={{ background:'var(--navy-light)', borderRadius:12, padding:'16px 20px', marginBottom:24, display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))', gap:12 }}>
        {[{icon:'📏',label:'Distance',val:'320 km'},{icon:'⏱',label:'Drive time',val:'8–9 hrs'},{icon:'🏔️',label:'Route',val:'Via Devprayag → Joshimath'},{icon:'ℹ️',label:'Note',val:'Motor road goes all the way to Badrinath…'}].map(s=>(
      ...
      <p style={{...p}}>
        <strong>Rishikesh → Devprayag → Rudraprayag → Chamoli → Joshimath → Badrinath</strong><br/>
        <span style={{ fontSize:13.5, color:'var(--text-muted)' }}>Motor road goes all the way to Badrinath temple (3,133m). No trek needed.</span>
      </p>
...
<div style={{ display:'flex', gap:10, padding:'10px 14px', background:'#fff', borderRadius:9, border:'1px solid var(--border)', fontSize:13.5, color:'var(--text-mid)', lineHeight:1.6 }}><span style={{ flexShrink:0 }}>💡</span>Check road status at Chamoli if visiting June–September (landslide risk)</div>
      ...
      <div style={{ background:'var(--navy-light)', borderRadius:12, padding:'16px 20px', marginBottom:24 }}>
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
          ...
          ].filter(l=>!l.href.includes('haridwar-to-badrinath-cab')).map(l=>(
      ...
      <div style={{ background:'var(--navy)', borderRadius:16, padding:'24px', textAlign:'center' }}>
        <h3 style={{ color:'#fff', fontFamily:'var(--font-display)', fontSize:'1.3rem', marginBottom:10 }}>Book Haridwar to Badrinath Cab</h3>
